          ...
          data.map((row) => (
            <TableRow key={row._id} className={cn("bg-transparent hover:bg-primary-700/10")}>
              {tableBody.map((cell, i) => (
                <>
                  {cell === "expiryDate" && (
                    <TableCell key={i} className={cn(itemVariants({ item: "expiry" }), "bg-transparent")}>
                      <Badge
                        className={cn(
                  ...
                  {cell === "name" && (
                    <TableCell key={i} className={cn(itemVariants({ item: "name" }), "bg-transparent")}>
                      <div className="text-primary-600 text-xs font-medium w-full">{row.manufacturer}</div>
                      <div className="w-full">{row.name}</div>
                    </TableCell>
                  )}
                  {cell === "description" && (
                    <TableCell key={i} className={cn(itemVariants({ item: "details" }), "bg-transparent")}>
                  ...
                  {cell === "price" && (
                    <TableCell
                      key={i}
                      className={cn(itemVariants({ item: "price" }), "bg-transparent flex items-center gap-x-0.5")}
                    >
                  ...
                  {cell === "stock" && (
                    <TableCell
                      key={i}
                      className={cn(itemVariants({ item: "stocks" }), "bg-transparent flex items-center gap-x-1")}
                    >
                      <div className="text-sm">{row.stock}</div>
              ...
              <TableCell className={cn("bg-transparent flex justify-center", itemVariants({ item: "action" }))}>
                <Actions row={row} disabled={formatExpiry(row.expiryDate) === "Expired"} />
              </TableCell>
            </TableRow>
          ))
        ) : (
