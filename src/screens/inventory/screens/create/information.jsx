...
const Information = ({ tabs, handleNext }) => {
  ...
  const validationSchema = Yup.object({
    ...
    manufacturer: Yup.string("Enter manufacturer").required(
  ...
  const formik = useFormik({
    initialValues: {
      name: current_tab.data?.name ?? "",
      description: current_tab.data?.description ?? "",
      manufacturer: current_tab.data?.manufacturer ?? "",
    },
    validationSchema: validationSchema,
    onSubmit: async (values, { resetForm, setFieldError }) => {
      setLoading(true);

      const res = await window.api.CreateProduct(values);
  ...
  return (
    <div className="flex-1 max-w-2xl">
      <div>
        ...
        <p className="text-sm text-primary-700">Create a product information</p>
      </div>
      ...
      <form className="space-y-3" onSubmit={formik.handleSubmit}>
        <div>
          ...
          <p className="mb-2 text-xs text-primary-700">Set the product name</p>
          <TextField
            type="text"
            name="name"
            id="name"
        ...
        <div>
          ...
          <p className="mb-2 text-xs text-primary-700">Set the manufacturer</p>
          <TextField
            type="text"
            name="manufacturer"
            id="manufacturer"
